...
const OperationsDashboard: React.FC<OperationsDashboardProps> = ({
    ...
    if (isTomorrow) {
      ...
      if (vipMeetingsTomorrow.length > 0) {
        return {
          ...
          icon: <AlertCircleIcon size={32} className="text-blue-600 dark:text-blue-400" />
        };
      }
      return null;
    } else if (isYesterdayView) {
      ...
      if (vipMeetingsYesterday.length > 0) {
        return {
          ...
          icon: <CheckCircleIcon size={32} className="text-gray-600 dark:text-gray-400" />
        };
      }
      return null;
    }
    // Only show for upcoming VIP meetings in the next hour
    const nextHourVipMeetings = upcomingMeetings.filter(m => m.isHighProfile && getMinutesUntilMeeting(m) <= 60);
    if (nextHourVipMeetings.length > 0) {
      const vipMeeting = nextHourVipMeetings[0];
      ...
      return {
        ...
        color: 'bg-red-50 text-red-800 dark:bg-red-900/20 dark:text-red-300 border border-red-300 dark:border-red-800',
        icon: vipMeeting.isCall ? <PhoneCallIcon size={32} className="text-red-600 dark:text-red-400" /> : <AlertCircleIcon size={32} className="text-red-600 dark:text-red-400" />
      };
    }
    return null;
  };
  ...
  return <div className="flex-1 overflow-auto flex flex-col h-full bg-gray-100 dark:bg-gray-900">
      ...
      <div className="p-4 flex-1 flex flex-col gap-4">
        {/* Alert bar moved to bottom - will be added later */}
        {/* Room status grid */}
        ...
        {/* Call Information Section - Content 20% larger */}
        {callInformation.length > 0 && <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-5">
            <div className="text-3xl font-bold mb-4 dark:text-white">CALLS</div>
            ...
            return <div key={`call-${index}`} className={`p-5 rounded-lg border-l-6 ${isVip ? 'bg-red-50 dark:bg-red-900/20 border-red-500' : 'bg-white dark:bg-gray-800 border-gray-300 dark:border-gray-700'}`}>
                    <div className="flex items-center justify-between">
                      <div className="flex-1">
                        <div className="flex items-center gap-4 flex-wrap">
                          ...
                          {isActive && <span className="ml-2 text-blue-600 dark:text-blue-400 font-bold text-xl bg-blue-50 dark:bg-blue-900/50 px-3 py-1.5 rounded">
                              ACTIVE
                            </span>}
                          {/* Call type with phone icon */}
                          <span className="text-gray-700 dark:text-gray-300 text-2xl flex items-center">
                            {call.callType || 'Conference'}
                            <PhoneCallIcon size={24} className="ml-3 text-blue-600 dark:text-blue-400" />
                          </span>
                          {/* Audience */}
                          ...
                          <span className={`ml-auto px-4 py-2 rounded-full font-bold text-lg ${isSecure ? 'bg-green-100 dark:bg-green-900/50 text-green-800 dark:text-green-300' : 'bg-yellow-100 dark:bg-yellow-900/50 text-yellow-800 dark:text-yellow-300'}`}>
                            {isSecure ? 'SECURE' : 'NON-SECURE'}
                          </span>
                        </div>
                        {/* VIP indicator with star icon */}
                        {isVip && <div className="flex items-center gap-3 mt-3">
                            <StarIcon size={28} className="text-red-600 dark:text-red-400" />
        ...
        {/* Simplified Upcoming Activity Section - Content 20% larger */}
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-5">
          <div className="text-3xl font-bold mb-4 dark:text-white">
            {isTomorrow ? 'SCHEDULED ACTIVITY' : isYesterdayView ? 'PAST ACTIVITY' : 'UPCOMING (NEXT 3 HOURS)'}
          </div>
          {upcomingActivities.length > 0 ? <div className="space-y-4">
            ...
            return <div key={index} className={`p-5 rounded-lg border-l-6 ${isVip ? 'bg-red-50 dark:bg-red-900/20 border-red-500' : 'bg-white dark:bg-gray-800 border-gray-300 dark:border-gray-700'}`}>
                    <div className="flex items-center justify-between">
                      <div className="flex-1">
                        <div className="flex items-center gap-4 flex-wrap">
                          {/* Larger time display */}
                          ...
                          <span className="text-gray-600 dark:text-gray-400 text-2xl mx-1">
                            |
                          </span>
                          ...
                          {isCall && <PhoneCallIcon size={32} className="ml-3 text-blue-600 dark:text-blue-400" />}
                          {/* VIP indicator with larger star icon */}
                          {isVip && <span className="ml-4 text-red-600 dark:text-red-400 font-bold flex items-center">
                              <StarIcon size={28} className="mr-2" />
                              <span className="text-2xl">VIP</span>
                            </span>}
                          {/* Larger secure/non-secure tag */}
                          {isCall && <span className={`ml-auto px-4 py-2 rounded-full font-bold text-lg ${isSecure ? 'bg-green-100 dark:bg-green-900/50 text-green-800 dark:text-green-300' : 'bg-yellow-100 dark:bg-yellow-900/50 text-yellow-800 dark:text-yellow-300'}`}>
                              {isSecure ? 'SECURE' : 'NON-SECURE'}
                            </span>}
                        </div>
                      </div>
            ...
            </div> : <div className="text-xl text-center py-5 text-gray-500 dark:text-gray-400">
        ...
        {/* VIP Alert moved to bottom with subtle styling */}
        {alertStatus && <div className={`${alertStatus.color} rounded-lg shadow-md p-4 flex items-center justify-between`}>
            <div className="flex items-center gap-4">
              {alertStatus.icon}
              <span className="text-2xl font-bold">{alertStatus.message}</span>
            </div>
          </div>}
